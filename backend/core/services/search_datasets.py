...
def search_by_title(title, page=0):
    ...
    url = f"https://datos.gob.es/apidata/catalog/dataset/title/{title}?_sort=title&_pageSize=200&_page={page}"
    headers = {
...
def search_by_keyword(keyword, page=0):
    """Consulta la API de datos.gob.es buscando datasets por keyword con paginación"""
    url = f"https://datos.gob.es/apidata/catalog/dataset/keyword/{keyword}?_sort=title&_pageSize=200&_page={page}"
    headers = {
        "Accept": "application/json",
        "User-Agent": "TFG Buscador de Datos - Estudiante"
    }
...
def search_by_spatial(spatial_type, spatial_value, page=0):
    """Consulta la API de datos.gob.es buscando datasets por tipo y valor espacial con paginación"""
    url = f"https://datos.gob.es/apidata/catalog/dataset/spatial/{spatial_type}/{spatial_value}?_sort=title&_pageSize=200&_page={page}"
    headers = {
        "Accept": "application/json",
        "User-Agent": "TFG Buscador de Datos - Estudiante"
    }
