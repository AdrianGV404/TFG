        ...
        <tbody>
            @foreach ($users as $user)
                <tr>
                    ...
                    <td>
                        @if($editingUserId === $user->id)
                            <input type="password" wire:model.defer="editingPassword" placeholder="Nueva contraseña (opcional)" class="form-control mb-1">
                        ...
                        @else
                            <button wire:click="startEdit({{ $user->id }})" class="btn btn-sm btn-primary">Editar</button>
                            <button wire:click="deleteUser({{ $user->id }})" class="btn btn-sm btn-danger">Eliminar</button>
                        @endif
                    </td>
                </tr>
            @endforeach
