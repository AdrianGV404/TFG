{{-- ============================================================
     ...
     Bloques:
       ...
       4. Estadísticas por Etiquetas | Últimas 15 tareas modificadas
     ============================================================ --}}
...
.task-time-list, .label-stats-list, .recent-list-container {
    max-height: 380px;
    overflow-y: auto;
...
.recent-list-container::-webkit-scrollbar { width: 5px; }
...
.ttl-id { font-size: .7rem; color: var(--db-muted); min-width: 32px; }
.ttl-title { flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 500; }
.ttl-hours { font-size: .78rem; font-weight: 700; color: var(--db-accent); white-space: nowrap; }
...
.pie-legend-count { font-weight: 700; color: var(--db-text); min-width: 26px; text-align: right; }
.pie-legend-pct { color: var(--db-muted); font-size: .72rem; min-width: 36px; text-align: right; }
...
.hm-grid::-webkit-scrollbar-thumb { background: var(--db-border); border-radius: 2px; }
.hm-col { display: flex; flex-direction: column; gap: 3px; flex-shrink: 0; }
.hm-col-label { font-size: .58rem; color: var(--db-muted); text-align: center; height: 14px; line-height: 14px; white-space: nowrap; }

.hm-cell {
    width: 13px; height: 13px; border-radius: 2px; cursor: pointer; position: relative; flex-shrink: 0; transition: transform .1s;
}
...
#hm-float-tooltip {
    display: none; position: fixed; background: var(--db-card); border: 1px solid var(--db-border);
    border-radius: 8px; box-shadow: 0 8px 24px rgba(0,0,0,.18); padding: 8px 11px; min-width: 180px;
    max-width: 260px; z-index: 9999; pointer-events: none; font-size: .73rem; color: var(--db-text); white-space: normal;
}
.hm-tooltip-date { font-weight: 700; margin-bottom: 5px; color: var(--db-accent); font-size: .75rem; }
.hm-tooltip-row { display: flex; justify-content: space-between; gap: 8px; padding: 2px 0; border-bottom: 1px solid var(--db-border); }
.hm-tooltip-row:last-child { border-bottom: none; }
.hm-tooltip-task { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 130px; }
.hm-tooltip-hours { font-weight: 700; color: var(--db-accent); white-space: nowrap; }

.hm-legend { display: flex; align-items: center; gap: 5px; margin-top: 10px; font-size: .7rem; color: var(--db-muted); }
...
.label-stat-card:hover { border-color: var(--db-accent); }
.label-stat-card:last-child { margin-bottom: 0; }
...
/* ── Recent tasks ───────────────────────────────────────── */
.recent-row {
    display: flex; align-items: center; gap: 12px; padding: 10px 14px; border-radius: 8px; cursor: pointer; transition: background .12s; text-decoration: none; color: inherit;
}
.recent-row:hover { background: rgba(59,130,246,.06); }
.recent-status-dot { width: 9px; height: 9px; border-radius: 50%; flex-shrink: 0; }
.recent-title { flex: 1; font-size: .85rem; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.recent-meta { font-size: .72rem; color: var(--db-muted); white-space: nowrap; }
.recent-project { font-size: .7rem; color: var(--db-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 130px; }

/* ── Loading overlay ────────────────────────────────────── */
.db-loading { display: flex; align-items: center; gap: 8px; font-size: .8rem; color: var(--db-muted); }
</style>
                            ...
                            <div class="label-stat-card">
                                <div class="label-stat-header">
                                    <span class="label-stat-name">
                                        <i class="fas fa-tag me-1" style="color:var(--db-accent); font-size:.75rem;"></i>
                                        {{ $ls['name'] }}
                                    </span>
                                    <span class="label-stat-meta">
                                        {{ $ls['total_hours'] }}h <span style="font-weight:400; color:var(--db-muted);">· {{ $ls['total_tasks'] }} tareas</span>
                                    </span>
                                </div>
                                <div class="label-stat-statuses">
                                    @if(($ls['statuses']['pending'] ?? 0) > 0)
                                        <span class="stat-badge"><span style="color:#f59e0b;">●</span> {{ $ls['statuses']['pending'] }} Pendientes</span>
                                    @endif
                                    @if(($ls['statuses']['in_progress'] ?? 0) > 0)
                                        <span class="stat-badge"><span style="color:#3b82f6;">●</span> {{ $ls['statuses']['in_progress'] }} En curso</span>
                                    @endif
                                    @if(($ls['statuses']['testing'] ?? 0) > 0)
                                        <span class="stat-badge"><span style="color:#8b5cf6;">●</span> {{ $ls['statuses']['testing'] }} En pruebas</span>
                                    @endif
                                    @if(($ls['statuses']['on_hold'] ?? 0) > 0)
                                        <span class="stat-badge"><span style="color:#6b7280;">●</span> {{ $ls['statuses']['on_hold'] }} En pausa</span>
                                    @endif
                                    @if(($ls['statuses']['done'] ?? 0) > 0)
                                        <span class="stat-badge"><span style="color:#22c55e;">●</span> {{ $ls['statuses']['done'] }} Completadas</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="recent-list-container">
                    @forelse($recentTasks as $task)
                        ...
                        <a href="{{ route('tasks.show', $task) }}" class="recent-row">
                            <div class="recent-status-dot" style="background:{{ $color }};" title="{{ $label }}"></div>
