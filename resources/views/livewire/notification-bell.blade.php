<div wire:poll.10s="loadNotifications" class="position-relative" x-data>
    ...
    {{-- ── Botón campana ── --}}
    <button wire:click="toggle" class="btn btn-dark position-relative notif-bell-btn"
    ...
    @if ($open)
        <div class="notif-dropdown shadow-lg"
         ...
         ">
            ...
            {{-- Header --}}
            <div
                style="padding:14px 18px; border-bottom:1px solid var(--db-card-border, #e2e8f0);
            ...
            <div style="max-height:420px; overflow-y:auto;">
                @forelse($notifications as $notif)
                    <div wire:click="markAsRead({{ $notif->id }})" wire:key="notif-{{ $notif->id }}"
                        style="
                        padding:12px 16px;
                    ...
                    ">
                        ...
                        <div style="flex:1; min-width:0;">
                            <div
                                style="font-size:.8rem; font-weight:{{ $notif->read_at ? '500' : '700' }};
                                    color:var(--db-text, #1e293b); margin-bottom:2px;
                                    white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                {{ $notif->title }}
                            </div>
                            <div
                                style="font-size:.72rem; color:var(--db-muted, #64748b); margin-bottom:3px;
